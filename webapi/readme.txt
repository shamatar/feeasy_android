...
(!) Касается случая, когда оплата производится по токену.
Если error - true, то нужно проверить значение результата err_name.
Если оно ERR_SENDER_TOKEN_NOT_FOUND, то значит токен отправителя заблокирован,
необходимо удалить его из сохранённых карт и спрашивать номер карты при
повторении этой операции.
...
Может указываться необязательный параметр user_id, тогда ответ содержит поле history_id, значение которого следует передать в transfer
...
sender_message - сообщение отправителя (необязательно). Макс - 2000 байт.
...
Если есть history_id, то он добавляется в запрос с обязательным указанием user_id. Без user_id он недействителен.
