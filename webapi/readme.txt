...
Может указываться необязательный параметр userId, тогда ответ содержит поле historyId, значение которого следует передать в transfer
...
Если есть historyId, то он добавляется в запрос с обязательным указанием userId. Без userId он недействителен.
